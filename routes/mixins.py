...
# Returns a list of errors raised by a form, 
#  which we'll pass to the front-end.
def FormErrors(*args):
    message = ''
    for f in args:
        if f.errors:
...
# Makes a POST request to the ReCaptcha API endpoint,
#  and stores the value in results.
...
# Redirects the user's to the JSON response from reCpatcha
...
# Retrieves the derection parameters from the request
